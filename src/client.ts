...
type ResultOfFunction<TFunc> = TFunc extends {
  handler: (ctx: any, args: any) => infer TResult;
}
  ? Awaited<TResult>
  : never;
...
export type ResultOfRef<TRef> = TRef extends { __result: infer TResult } ? TResult : never;
...
export type ConvoyClient = {
  baseUrl?: string;
  query: <TRef extends QueryRef<string, any, any>>(ref: TRef, args: ArgsOfRef<TRef>) => Promise<ResultOfRef<TRef>>;
...
export function createConvoyClient(options?: ConvoyClientOptions): ConvoyClient {
  const baseUrl = normalizeBaseUrl(options?.baseUrl ?? '');
  return {
    baseUrl,
    query: async <TRef extends QueryRef<string, any, any>>(ref: TRef, args: ArgsOfRef<TRef>) => {
      if (ref.kind !== 'query') {
        throw new Error('Expected a query reference');
      }
      return callApi<ResultOfRef<TRef>>('query', ref.name, args, options);
    },
    mutation: async <TRef extends MutationRef<string, any, any>>(ref: TRef, args: ArgsOfRef<TRef>) => {
      if (ref.kind !== 'mutation') {
        throw new Error('Expected a mutation reference');
      }
      return callApi<ResultOfRef<TRef>>('mutation', ref.name, args, options);
    },
  };
}

export function makeQueryRef<Name extends string, TFunc>(
  name: Name,
  _fn: TFunc,
): QueryRef<Name, ArgsOfFunction<TFunc>, ResultOfFunction<TFunc>> {
  return { kind: 'query', name } as QueryRef<Name, ArgsOfFunction<TFunc>, ResultOfFunction<TFunc>>;
}

export function makeMutationRef<Name extends string, TFunc>(
  name: Name,
  _fn: TFunc,
