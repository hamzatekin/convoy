...
describe('server functions', () => {
  it('parses query input and returns handler result', async () => {
    ...
    await expect(fn.run({}, { count: 2 })).resolves.toBe(3);
    await expect(fn.run({}, { count: '2' })).rejects.toThrow();
  });
  ...
  it('parses mutation input and returns handler result', async () => {
    ...
    await expect(fn.run({}, { name: 'alpha' })).resolves.toBe('ALPHA');
    await expect(fn.run({}, { name: 123 })).rejects.toThrow();
  });
